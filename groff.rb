...
class Groff < Formula
  ...
  # TODO: Add option for using system's ghostscript instead of installing homebrewed version. That
  # allows this to play nice with, e.g., MacTex (which install its own ghostscript.)
  # The ./configure option --with-gs=PATH may be of use. We could also set ghostscript as build-only
  # dependency, though that may cause groff to always look at that (temp) path for gs.
  ...
  # To support HTML/XHTML output, ./configure looks for `pnmcut', `pnmcrop', `pnmtopng', `psselect',
  # and `pnmtops'. HTML/XHTML may still work without these, but only if the output doesn't
  # include tables, images, or other graphical elements.
  depends_on 'netpbm' if build.with? 'grohtml'
  depends_on 'psutils' if build.with? 'grohtml'

  def patches
    # The patched bug is only hit if --with-grohtml is used, cuasing configure to find pnmtops
